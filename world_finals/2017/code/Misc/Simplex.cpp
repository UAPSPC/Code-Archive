...
void pivot(int m, int n, int a, int b) {
  ...
  for(i = 0; i <= m; i++)
    if(i != a)
      for(j = 0; j <= n; j++)
        if(j != b) ine[i][j] -= ine[a][j] * ine[i][b] / ine[a][b];
  for(j = 0; j <= n; j++)
...
double simplex(int m, int n, double c[][maxn], double x[]) {
  int i, j, ii, jj;
  for(i = 1; i <= m; i++)
    for(j = 0; j <= n; j++) ine[i][j] = c[i][j];
  for(j = 0; j <= n; j++) ine[0][j] = -c[0][j];
  for(i = 0; i <= m; i++) basis[i] = -i;
  for(j = 0; j <= n; j++) out[j] = j;
  for(;;) {
    for(i = ii = 1; i <= m; i++)
      if(ine[i][n] < ine[ii][n] || (ine[i][n] == ine[ii][n] && basis[i] < basis[ii])) ii = i;
    if(ine[ii][n] >= -eps) break;
    for(j = jj = 0; j < n; j++)
      if(ine[ii][j] < ine[ii][jj] - eps || (ine[ii][j] < ine[ii][jj] - eps && out[i] < out[j])) jj = j;
  ...
  for(j = 0; j < n; j++) x[j] = 0;
  for(i = 1; i <= m; i++)
    if(basis[i] >= 0) x[basis[i]] = ine[i][n];
  return ine[0][n];
}
