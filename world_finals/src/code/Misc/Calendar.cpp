...
int leapYears(int from, int to) { // [from, to)
  ...
  if(isLeap(from)) return fours - hundreds + fhundreds + 1;
...
int dateToDay(Date date) {
  ...
  for(int i = 0; i < month; i++) days += getMonthDays(year, i);
  days += day;
  return days;
}
Date dayToDate(int days) {
  ...
  days -= leapYears(epoch, year);
  int month = 0;
  for(; month < 12 && days > getMonthDays(year, month); month++) days -= getMonthDays(year, month);
