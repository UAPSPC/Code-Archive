...
inline vector<point> convexhull(vector<point> &pts) {
  ...
  for(int i = 0; i < (int)pts.size(); i++) {
    ...
    while(lower.size() >= 2 && cross(lower.back() - lower[lower.size() - 2], pts[i] - lower.back()) < eps) lower.pop_back();
    // >eps include all points on border
    while(upper.size() >= 2 && cross(upper.back() - upper[upper.size() - 2], pts[i] - upper.back()) > -eps) upper.pop_back();
