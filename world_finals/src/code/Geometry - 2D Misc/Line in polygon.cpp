...
const double Lowest = -10000000; const double Highest = 10000000;
const point InvalidLowerBound = point(Lowest - 1, Lowest - 1);
const point InvalidUpperBound = point(Highest + 1, Highest + 1);
...
// Given the line a, will return points just above and just below base on the line.
...
// Allows arbitrary points and will return the two furthest points reachable from a.
// If returns InvalidLowerBound or InvalidUpperBound, then the line starting from a is partially
// outside.
pair<point, point> line_polygon_intersections(point a, point b, const vector<point>& polygon) {
  ...
  for (int edge = 0; edge < polygon.size(); ++edge) {
    ...
    bool exitsLower = true; bool exitsUpper = true;
    ...
    if (exitsLower && LE(p, a) && GE(p, aLowerMost)) {
      aLowerMost = p;
    }
    if (exitsUpper && GE(p, a) && LE(p, aUpperMost)) {
      aUpperMost = p;
    }
  }
